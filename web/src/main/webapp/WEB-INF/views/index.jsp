<div class="container">

    <div class="row">
        <div class="col-md-3">
            <div class="sidebar-content">
                <div id="btn-calibrate" class="btn btn-primary btn-block">Calibrate</div>
                <div id="btn-mock" class="btn btn-success btn-block">Change to mock</div>
            </div>
        </div>
        <div class="col-md-9">
            <div class="canvas-container-content">
                <canvas id="canvas-seat-control" width="600" height="600"></canvas>
                <div id="image-container">
                    <img src="#" style="display:none">
                </div>
                <div id="slider-top-container">
                    <div id="slider-top"></div>
                </div>
                <div id="slider-header-h-container">
                    <div id="slider-header-h" class="hidden-slider"></div>
                </div>
                <div id="slider-header-v-container">
                    <div id="slider-header-v" class="hidden-slider"></div>
                </div>
                <div id="slider-middle-h-container">
                    <div id="slider-middle-h" class="hidden-slider"></div>
                </div>
                <div id="slider-middle-v-container">
                    <div id="slider-middle-v" class="hidden-slider"></div>
                </div>
                <div id="slider-bottom-h-container">
                    <div id="slider-bottom-h" class="hidden-slider"></div>
                </div>
                <div id="slider-bottom-v-head-container">
                    <div id="slider-bottom-v-head" class="hidden-slider"></div>
                </div>
                <div id="slider-bottom-v-tail-container">
                    <div id="slider-bottom-v-tail" class="hidden-slider"></div>
                </div>
                <div id="slider-bottom-container">
                    <div id="slider-bottom"></div>
                </div>
                <a id="btn-head" href="javascript:void(0)" class="btn-main"><i class="glyphicon glyphicon-plus"></i></a>
                <a id="btn-middle" href="javascript:void(0)" class="btn-main"><i class="glyphicon glyphicon-plus"></i></a>
                <a id="btn-bottom" href="javascript:void(0)" class="btn-main"><i class="glyphicon glyphicon-plus"></i></a>
            </div>
        </div>
    </div>
    <div class="row below">
        <div class="col-md-12">
            <p>System logs:</p>
            <textarea id="textarea-log" class="form-control" rows="20" disabled></textarea>
            <br>
        </div>
    </div>
</div>
